...
Discussion phase. UI stack accepted (decision 0002: web-only React + TypeScript + Vite); combined product shell, docking workbench layout, and first-slice workflow accepted (decisions 0003-0006); package selection proposed in docs/package-selection.md, not yet accepted; nothing scaffolded, so no build, tests, or preview exist yet. Use `git status --short --branch`, `git diff --check`, `git diff --cached`, and `git log --oneline -5`. Update this section and the README when actual commands become known. Do not scaffold the primary prototype before the discussion checkpoint.
