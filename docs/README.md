...
Current phase (2026-09-18): the agentic presentation demo on the `feature/agentic` branch ([decision 0020](decisions/0020-agentic-presentation-demo.md)). A language model drives the workbench's presentation, styling, and layout through the same operations a person uses, over four fictional districts, with no backend: the key stays on the dev server and a built bundle has no route to a model. History: setup and source audit complete; UI stack accepted in [decision 0002](decisions/0002-web-react-typescript-vite.md); combined shell, workbench layout, and first-slice workflow in decisions [0003](decisions/0003-combined-product-shell.md), [0004](decisions/0004-workbench-layout-and-docking.md), and [0005](decisions/0005-first-slice-workflow-and-panel-scope.md); scripted agent in [0006](decisions/0006-scripted-agent-and-layout-details.md); packages in [0007](decisions/0007-package-selection.md) and FlexLayout in [0008](decisions/0008-flexlayout-docking.md); design guidance in [0009](decisions/0009-ui-design-guidelines.md) and [0010](decisions/0010-geist-typeface.md); the basemap, 3D buildings, terrain, and lighting in [0012](decisions/0012-openfreemap-basemap-back-bay.md), [0013](decisions/0013-3d-building-extrusion.md), [0015](decisions/0015-terrain.md), [0018](decisions/0018-map-scene-lighting.md), and [0019](decisions/0019-globe-projection-and-solar-state.md); the Reasoning and Inspection panels in [0016](decisions/0016-reasoning-and-inspection-panels.md); and GitHub Pages in [0017](decisions/0017-github-pages-deployment.md).
